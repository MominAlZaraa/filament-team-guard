...
class CreateApiToken extends BaseLivewireComponent
{
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->schema([
                Section::make(__('filament-team-guard::default.create_api_token.section.title'))
                    ->aside()
                    ->description(__('filament-team-guard::default.create_api_token.section.description'))
                    ->schema([
                        TextInput::make('name')
                            ->label(__('filament-team-guard::default.form.token_name.label'))
                            ->minLength(3)
                            ->required()
                            ->string()
                            ->maxLength(255),
                        TextEntry::make('permissions')
                            ->label(__('filament-team-guard::default.form.permissions.label'))
                            ->state(''),
                        Grid::make()
                            ->columns()
                            ->schema(
                        ...
                        Actions::make([
                            Action::make('save')
                                ->label(__('filament-team-guard::default.action.create_token.label'))
                                ->submit('createToken'),
                        ]),
                    ]),
            ])
    ...
    public function createToken(): void
    {
        ...
        if (empty($permissions)) {
            Notification::make()
                ->danger()
                ->body(__('filament-team-guard::default.notification.create_token.error.message'))
                ->send();
        ...
        Notification::make('showApiToken')
            ...
            ->title(__('filament-team-guard::default.notification.create_token.success.message'))
            ->actions([
                Action::make('copy_token_confirm')
                    ->label(__('filament-team-guard::default.action.copy_token_confirm.label'))
    ...
    {
        return view('filament-team-guard::livewire.api-tokens.create-api-token');
    }
    ...
    public function sendTokenCopiedNotification(): void
    {
        Notification::make()
            ->success()
            ->body(__('filament-team-guard::default.notification.copy_token.success.message'))
